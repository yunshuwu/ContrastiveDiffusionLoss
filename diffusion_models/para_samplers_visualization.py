"""
This is for 1D gaussian data, to visualize the dynamics. 
...
@torch.no_grad()
def parasam_forward(
    model: Any,                                         # on GPU:0! the pretrained diffusion model
    ...
    seed: int=42,                                       # manually set the random seed, for visualizing the trajectory of this certain 1D gaussian point
):
    """Main loop of ParaSampler."""
    print("parallel pipeline!", flush=True)

    # 1. Prepare sampling schedule
    schedule = scheduler.get_sampling_schedule(device=device) # NOTE: no matter which one is used (logsnr/sigma/timesteps), all of them are indexed by timestep! And use get_XXX() function to get values.
    

    # 2. Sample from prior ~ N(0,I), shape=[num_samples, *model.shape]
    torch.manual_seed(seed=seed) # to plot certain 1D point's trajectory
    x_init = torch.randn([num_samples, *model.shape], device=device)
    ...
    # for visualization
    trajectory_buffer = [] # every entry of this list will have shape = [len(schedule)+1, num_samples, *model.shape]
    trajectory_buffer.append(samples_time_evolution_buffer[:,0,:].clone())
    ...
    for j in range(len(schedule)): 
        torch.manual_seed(seed=j) # control all random noise to be the same for visualiaztion
    ...
    # We will be dividing the norm of the noise, so store its inverse here to avoid a division at every step
    inverse_variance_norm = 1. / torch.tensor([scheduler.get_variance(scheduler.schedule[j]) for j in range(len(schedule))] + [0]).to(noise_array.device) 
    sample_dim = model.d 
    inverse_variance_norm = inverse_variance_norm[:,None] / sample_dim 
    ...
    while begin_idx < len(schedule):
        ...
        block_t = scheduler.schedule[begin_idx:end_idx] 
        block_t = block_t[:,None].repeat(1, num_samples) 
        t_vec = block_t
        ...
        # print(f'    block_samples_new[{end_idx}] = {block_samples_new[-1]}')
        ...
        err_ratio = cur_err * inverse_variance_norm[begin_idx+1:end_idx+1] # calculate the inverse_var_norm=1/(var_j^2*D) since: err_j/D > (tau^2 * var_j^2). # TODO: inverse_var_norm.shape=[parallel_len, 1], inverse_var_norm[:,None] adds a new dim to the end
        ...
        # Visualization: need to store the current trajectory snapshot for visulizing how trajectory converges
        # if stats_pass_count < 2:
        trajectory_buffer.append(samples_time_evolution_buffer[:,0,:].clone())
    ...
    trajectory_buffer = torch.stack(trajectory_buffer, dim=0)
    ...
    return final_samples, stats, trajectory_buffer
...
def parasam_forward_mmd(
    ...
    tolerance: float = 0.1,                             # tolerance when picard iteration converges
    mmd_threshold: float = 4e-4,                        # NOTE: set this MMD threshold since "tolerance" controls both convergence speed and sample quality
    mp_queues: Optional[torch.FloatTensor] = None,      # multiprocess queues
    device: Optional[str] = None,                       # device for the main loop
    num_consumers: Optional[int] = int,                 # number of GPUs to be used
    seed: int=42,                                       # manually set the random seed, for visualizing the trajectory of this certain 1D gaussian point
    ...
    # for visualization
    trajectory_buffer = [] # every entry of this list will have shape = [len(schedule)+1, num_samples, *model.shape]
    trajectory_buffer.append(samples_time_evolution_buffer[:,0,:].clone()) # only take the first sample in the batch
    ...
    while begin_idx < len(schedule):
        ...
        def mmd(gt_samples, est_samples):
            bw = median_heuristic(gt_samples)
            ...
            return mmd_mean
        # print(f'    gt_samples.device = {true_samples.device}, est_samples.device = {samples_time_evolution_buffer.device}')
...
def parasam_forward_worker(
    ...
    while True:
        ...
        model_output_chunk = model(
            z[chunk].flatten(0,1).to(device), 
            logsnr_chunk.to(device), 
        ) # model_output is the estimated noise
