...
class WrapUNetModel(UNetModel):
    ...
    #     ''' 
    #     Yunshu: here the forward() should take logsnr instead of snr 
    #     '''
    ...
    def forward(self, x, logsnr):
        ''' 
        Yunshu: here the forward() takes logsnr
        The input "x" only contains data information, no conditional information y. (In diffusionmodel.py, batch=[x,y], here x corresponding to only batch[0]=x)
        '''
...
class WrapUNet2DModel(UNet2DModel):
    ...
    def forward(self, x, logsnr):
        ''' 
        Yunshu: here the forward() takes logsnr 
        The input x here only contains data info, no conditional class info
        '''
        timestep = self.logsnr2t(logsnr)
        eps_hat = super().forward(x, timestep).sample
        return eps_hat
    ...
    def logsnr2t(self, logsnr):
        ...
        timestep = th.argmin(abs(alphabarGT-alphas_cumprod.unsqueeze(1)), dim=1) * scale # bring the timestep back to the original range [0,T]
        return timestep


class UNet2DModel_DDPM(UNet2DModel):
    """Wrap UNet2DModel to accept arguments compatible with Diffusion Model."""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
